...
import {
	...
	sortPlayers,
	filterPlayers
} from '../../actions/player_actions.js';
...
class SortExample extends Component {
	constructor(props) {
		...
		this.onColumnResizeEndCallback = this.onColumnResizeEndCallback.bind(
			this
		);
	...
	onKeyDown(e) {
		if (!e.repeat) {
			...
			if (e.keyCode === 38) {
				...
				// down arrow
				e.preventDefault();
				const maxRow = this.getPlayerKeys(this.props.players.list)
					.length;
				console.log(maxRow);
				rowIndex += 5;
				if (rowIndex > maxRow) {
	...
	onColumnResizeEndCallback(newColumnWidth, columnKey) {
		if (window.location.pathname === '/edit') {
			...
			const headingNames = Object.values(this.props.headings).map(
				headingObject => {
					return headingObject.name;
				}
			);
		...
		if (window.location.pathname.indexOf('results') !== -1) {
			if (window.location.pathname.indexOf('all') === -1) {
				...
				possibleRowHeadings.map(possibleRowHeading => {
					if (
						rowHeadingsChosen.indexOf(possibleRowHeading.name) !==
						-1
					) {
			...
			return (
				<div>
					<Controls
						...
						onToggle={this.onScrollToggle}
						onClear={this.onClearFilterSearch}
					/>
					<Table
						rowHeight={40}
						rowsCount={rowsCount}
